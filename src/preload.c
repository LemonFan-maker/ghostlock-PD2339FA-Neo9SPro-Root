...
#define SU_TMPFS_MARKER ".su_tmpfs"

static int path_is_our_tmpfs(const char *path) {
  char marker[512];
  snprintf(marker, sizeof(marker), "%s/%s", path, SU_TMPFS_MARKER);
  return access(marker, F_OK) == 0;
}

static int path_looks_like_pristine_apex(const char *path) {
  char virtmgr[512];
  snprintf(virtmgr, sizeof(virtmgr), "%s/virtmgr", path);
  return access(virtmgr, F_OK) == 0;
}

static void mark_tmpfs(const char *path) {
  char marker[512];
  snprintf(marker, sizeof(marker), "%s/%s", path, SU_TMPFS_MARKER);
...
static void teardown_stale_su_tmpfs(void) {
  if (!path_is_mounted(SU_DST_DIR)) {
    return;
  }
  int stale = path_is_our_tmpfs(SU_DST_DIR) && access(SU_DST, F_OK) != 0;
  if (!stale && !path_looks_like_pristine_apex(SU_DST_DIR) &&
      access(SU_DST, F_OK) != 0) {
    stale = 1;
  }
  if (stale) {
    umount2(SU_DST_DIR, MNT_DETACH);
    pr_info("stale su tmpfs on %s detached\n", SU_DST_DIR);
  }
}

static int ensure_su_mount(void) {
  if (path_is_mounted(SU_DST_DIR)) {
    if (access(SU_DST, F_OK) == 0 || path_is_our_tmpfs(SU_DST_DIR)) {
      return 1;
    }
    pr_info("stacking su tmpfs over %s\n", SU_DST_DIR);
  }
  if (mount("tmpfs", SU_DST_DIR, "tmpfs", 0, "mode=0755,size=4m") == 0) {
    mark_tmpfs(SU_DST_DIR);
    return 1;
  }
  return errno == EBUSY;
}
...
static pid_t start_su_daemon(void) {
  ...
  if (pid == 0) {
    ...
    execl(SU_DST, "zygote_aux", "--daemon", (char *)NULL);
    _exit(127);
  }
  return pid;
}
