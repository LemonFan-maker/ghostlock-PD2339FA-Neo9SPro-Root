...
#else
#define pr_error(fmt, ...) do { \
        char _tsb[24]; pr_ts_into(_tsb, sizeof _tsb); \
        printf(COLOR_RED "%s [!] [%d:%ld] %s:%d " COLOR_DEFAULT fmt, _tsb, getpid(), syscall(SYS_gettid), __FILE__, __LINE__, ##__VA_ARGS__); \
        exit(-1); \
    } while (0)
#define pr_warning(fmt, ...) do { \
        char _tsb[24]; pr_ts_into(_tsb, sizeof _tsb); \
...
#define pr_info(fmt, ...) do { \
        char _tsb[24]; pr_ts_into(_tsb, sizeof _tsb); \
        printf(COLOR_YELLOW "%s [*] [%d:%ld] %s:%d " COLOR_DEFAULT fmt, _tsb, getpid(), syscall(SYS_gettid), __FILE__, __LINE__, ##__VA_ARGS__); \
    } while (0)
#define pr_success(fmt, ...) do { \
        char _tsb[24]; pr_ts_into(_tsb, sizeof _tsb); \
        printf(COLOR_GREEN "%s [+] [%d:%ld] %s:%d " COLOR_DEFAULT fmt, _tsb, getpid(), syscall(SYS_gettid), __FILE__, __LINE__, ##__VA_ARGS__); \
    } while (0)
#endif
#else
#ifdef ANDROID_APP_NO_LKM
...
#define pr_error(fmt, ...) do { \
        char _tsb[24]; pr_ts_into(_tsb, sizeof _tsb); \
        printf(COLOR_RED "%s [!] [%d:%ld] " COLOR_DEFAULT fmt, _tsb, getpid(), syscall(SYS_gettid), ##__VA_ARGS__); \
        fflush(stdout); \
        exit(-1); \
    } while (0)
#define pr_warning(fmt, ...) do { \
...
#define pr_info(fmt, ...) do { \
        char _tsb[24]; pr_ts_into(_tsb, sizeof _tsb); \
        printf(COLOR_YELLOW "%s [*] [%d:%ld] " COLOR_DEFAULT fmt, _tsb, getpid(), syscall(SYS_gettid), ##__VA_ARGS__); \
        fflush(stdout); \
    } while (0)
#define pr_success(fmt, ...) do { \
        char _tsb[24]; pr_ts_into(_tsb, sizeof _tsb); \
        printf(COLOR_GREEN "%s [+] [%d:%ld] " COLOR_DEFAULT fmt, _tsb, getpid(), syscall(SYS_gettid), ##__VA_ARGS__); \
        fflush(stdout); \
    } while (0)
#endif
#endif
