...
#ifndef PSELECT_STACK_PAD
#define PSELECT_STACK_PAD 0
#endif

#if PSELECT_STACK_PAD > 0
__attribute__((noinline))
static int padded_pselect(int nfds, fd_set *readfds, fd_set *writefds,
                          fd_set *exceptfds, const struct timespec *timeout,
                          const sigset_t *sigmask) {
  volatile char pad[PSELECT_STACK_PAD];
  pad[0] = 0;
  pad[PSELECT_STACK_PAD - 1] = 0;
  __asm__ volatile("" : : "r"(pad) : "memory");
  return pselect(nfds, readfds, writefds, exceptfds, timeout, sigmask);
}
#define exploit_pselect padded_pselect
#else
#define exploit_pselect pselect
#endif
