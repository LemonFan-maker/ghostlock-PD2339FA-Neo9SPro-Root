...
#define ASSERT_pr(cond, fmt, ...) \
  do { \
    if (!!(cond) == 0) \
      PR_ASSERT("[detected] assert(%s): " fmt, #cond, ##__VA_ARGS__); \
  } while (0)

static inline void pr_ts_into(char *buf, size_t n) {
  struct timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
  snprintf(buf, n, "[%5lu.%03lu]", (unsigned long)ts.tv_sec,
           (unsigned long)(ts.tv_nsec / 1000000));
}

static inline int gl_log_info_on(void) {
  static int on = -1;
  if (on < 0) {
    const char *e = getenv("GL_LOG_INFO");
    on = (e && e[0] && e[0] != '0') ? 1 : 0;
  }
  return on;
}
...
#define pr_error(fmt, ...) \
  do { \
    __android_log_print(ANDROID_LOG_ERROR, "google_poc_app", "[!] %s:%d " fmt, \
                        __FILE__, __LINE__, ##__VA_ARGS__); \
    exit(-1); \
  } while (0)
#define pr_warning(fmt, ...) \
  do { \
    __android_log_print(ANDROID_LOG_WARN, "google_poc_app", "[-] %s:%d " fmt, \
                        __FILE__, __LINE__, ##__VA_ARGS__); \
  } while (0)
#define pr_info(fmt, ...) \
  do { \
    if (!gl_log_info_on()) \
      break; \
    __android_log_print(ANDROID_LOG_INFO, "google_poc_app", "[*] %s:%d " fmt, \
                        __FILE__, __LINE__, ##__VA_ARGS__); \
  } while (0)
...
#define pr_error(fmt, ...) \
  do { \
    char _tsb[24]; \
    pr_ts_into(_tsb, sizeof _tsb); \
    printf(COLOR_RED "%s [!] [%d:%ld] %s:%d " COLOR_DEFAULT fmt, _tsb, \
...
#define pr_info(fmt, ...) \
  do { \
    ...
    char _tsb[24]; \
    pr_ts_into(_tsb, sizeof _tsb); \
    printf(COLOR_YELLOW "%s [*] [%d:%ld] %s:%d " COLOR_DEFAULT fmt, _tsb, \
           getpid(), syscall(SYS_gettid), __FILE__, __LINE__, ##__VA_ARGS__); \
  } while (0)
#define pr_success(fmt, ...) \
  do { \
    char _tsb[24]; \
    pr_ts_into(_tsb, sizeof _tsb); \
    printf(COLOR_GREEN "%s [+] [%d:%ld] %s:%d " COLOR_DEFAULT fmt, _tsb, \
           getpid(), syscall(SYS_gettid), __FILE__, __LINE__, ##__VA_ARGS__); \
  } while (0)
#endif
#else
...
#define pr_error(fmt, ...) \
  do { \
    __android_log_print(ANDROID_LOG_ERROR, "google_poc_app", "[!] " fmt, \
                        ##__VA_ARGS__); \
    exit(-1); \
  } while (0)
#define pr_warning(fmt, ...) \
  do { \
    __android_log_print(ANDROID_LOG_WARN, "google_poc_app", "[-] " fmt, \
                        ##__VA_ARGS__); \
  } while (0)
#define pr_info(fmt, ...) \
  do { \
    if (!gl_log_info_on()) \
      break; \
    __android_log_print(ANDROID_LOG_INFO, "google_poc_app", "[*] " fmt, \
                        ##__VA_ARGS__); \
  } while (0)
#define pr_success(fmt, ...) \
  do { \
...
#define pr_error(fmt, ...) \
  do { \
    char _tsb[24]; \
    pr_ts_into(_tsb, sizeof _tsb); \
    printf(COLOR_RED "%s [!] [%d:%ld] " COLOR_DEFAULT fmt, _tsb, getpid(), \
...
#define pr_info(fmt, ...) \
  do { \
    ...
    char _tsb[24]; \
    pr_ts_into(_tsb, sizeof _tsb); \
    printf(COLOR_YELLOW "%s [*] [%d:%ld] " COLOR_DEFAULT fmt, _tsb, getpid(), \
           syscall(SYS_gettid), ##__VA_ARGS__); \
    fflush(stdout); \
  } while (0)
#define pr_success(fmt, ...) \
  do { \
    char _tsb[24]; \
    pr_ts_into(_tsb, sizeof _tsb); \
    printf(COLOR_GREEN "%s [+] [%d:%ld] " COLOR_DEFAULT fmt, _tsb, getpid(), \
           syscall(SYS_gettid), ##__VA_ARGS__); \
    fflush(stdout); \
  } while (0)
#endif
